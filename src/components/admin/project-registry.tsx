...
export default function VendorRequestsRegistry() {
  ...
  useEffect(() => {
    fetch("https://jadd-production-275a.up.railway.app/admin/Identitie")
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
  ...
  const handleAction = async (item: any, action: string) => {
    if (action === "delete") {
      await fetch(`https://jadd-production-275a.up.railway.app/admin/Identitie/${item._id}`, { method: "DELETE" });
      setRequests(requests.filter((r) => r._id !== item._id));
    } else {
      const newStatus = action === 'activate' ? 'verified' : 'unverified';
      try {
        const response = await fetch(`https://jadd-production-275a.up.railway.app/admin/status/${item.userId._id}`, {
          method: "PUT",
